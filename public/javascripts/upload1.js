var Upload = function() {
	...
	'this.startUpload()'
     */
        this.file_dialog_complete = function(num_selected, num_queued)
        {
                if (num_queued > 0)
                {
                        _this.files_queued += num_queued;
                        $('upload').value = 'Upload ' + _this.files_queued + '  files';
                        $('progress').innerHTML ='';
                }
        }
     ...
     */
        ...
        {
                new Insertion.Bottom('selected_files', "<li class='"+file.id+"'>"+file.name+" <small class='"+file.id+"_percent' style='color:red'></small></li>");
        }
	...
	uploadError event to fired.
       */
        ...
        {                
                $('upload').disable();
                $('progress').innerHTML = "Uploading...please be patient!";
        }
	...
	been uploaded. This is a bug in the Linux Flash Player that we cannot work around.
       */
        this.upload_progress=function(file, bytes, total)
        {
                percent = Math.ceil((bytes / total) * 100);
                $('selected_files').down("."+file.id+"_percent").innerHTML = " - <small>"+percent + "% uploaded.</small>";
        }
	...
	Upload error will not fire for files that are cancelled but still waiting in the queue.
       */
        this.upload_error=function(file, code, message)
        {
	...
	uploadSuccess.
     */
        this.upload_success=function(file, data)
        {
                $('selected_files').down("."+file.id+"_percent").innerHTML = " - done";
        }
     ...
     */
        ...
        {
                _this.files_queued -= 1;

                if (_this.files_queued == 0)
                {
                        $('upload').enable();
                        $('upload').value = 'upload';
                        $('progress').innerHTML ='Upload complete!';

                        location = "./edit_images";
                }
     ...
     */
        this.swf_upload_loaded= function() {}
